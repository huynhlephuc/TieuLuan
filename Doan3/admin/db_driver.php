...
class DB_driver
{
    ...
    function insert($table, $data)
    {
		...
		if (mysqli_query($this->__conn, $sql)) {
			$last_id = mysqli_insert_id($this->__conn);
		  }
...
{    
	...
	if ($current_page > 1 && $total_page > 1){
		$html .= '<a href="'.str_replace('{page}', $current_page-1, $link).'"class = "w3-bar-item w3-button w3-red w3-hover-black">Prev</a>';
	}
	...
	for ($i = 1; $i <= $total_page; $i++){
		...
		if ($i == $current_page){
			$html .= '<span class = "w3-bar-item w3-btn w3-white w3-border w3-border-red w3-hover-black">'.$i.'</span>';
		}
		else{
			$html .= '<a href="'.str_replace('{page}', $i, $link).'" class = "w3-bar-item w3-button w3-red w3-hover-black">'.$i.'</a>';
		}
	}
 
	// nếu current_page < $total_page và total_page > 1 mới hiển thị nút prev
	if ($current_page < $total_page && $total_page > 1){
		$html .= '<a href="'.str_replace('{page}', $current_page+1, $link).'" class = "w3-bar-itemw w3-button w3-red w3-hover-black">Next</a>';
	}
